...
public class FilePersistanceReceipt {
    ...
    public String save(Visit v, Date paymentDate) throws Exception {
        ...
        Double totalProduct = v.getProductExpense();
        Double totalService = v.getServiceExpense();
        Double totalWithoutDiscount = v.getTotalExpenseWithoutDiscount();
        Double totalWithDiscount = v.getTotalExpense();

        String paymentReformatted = paymentDate.toString().replace(':', '-');

        String pathtoreceipt = "\\Receipt - " + c.getName() + paymentReformatted + ".txt";
